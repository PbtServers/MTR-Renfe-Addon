...
public class ExtendedItemRailModifier extends ItemNodeModifierBase {
    ...
    public ExtendedItemRailModifier(ItemSettings itemSettings) {
        ...
        isOneWay = false;
        extendedRailType = null;
    }

    public ExtendedItemRailModifier(boolean forNonContinuousMovementNode, boolean forContinuousMovementNode, boolean forAirplaneNode, boolean isOneWay, ExtendedRailType extendedRailType, ItemSettings itemSettings) {
    ...
    public void addTooltips(ItemStack stack, @Nullable World world, List<MutableText> tooltip, TooltipContext options) {
        if (isConnector && extendedRailType != null && extendedRailType.canAccelerate) {
            tooltip.add(TranslationProvider.TOOLTIP_MTR_RAIL_SPEED_LIMIT.getMutableText(extendedRailType.speedLimit).formatted(TextFormatting.GRAY));
        }
    ...
    public Rail createRail(@Nullable UUID uuid, TransportMode transportMode, BlockState stateStart, BlockState stateEnd, BlockPos posStart, BlockPos posEnd, Angle facingStart, Angle facingEnd) {
        if (extendedRailType != null && uuid != null) {
            final boolean isValidContinuousMovement;
            final ExtendedRailType newRailType;
            ...
            if (transportMode.continuousMovement) {
                ...
                if (blockStart.data instanceof BlockNode.BlockContinuousMovementNode && blockEnd.data instanceof BlockNode.BlockContinuousMovementNode && ((BlockNode.BlockContinuousMovementNode) blockStart.data).isStation && ((BlockNode.BlockContinuousMovementNode) blockEnd.data).isStation) {
                    ...
                    newRailType = extendedRailType.isSavedRail ? extendedRailType : ExtendedRailType.CABLE_CAR_STATION;
                } else {
                    final int differenceX = posEnd.getX() - posStart.getX();
                    final int differenceZ = posEnd.getZ() - posStart.getZ();
                    isValidContinuousMovement = !extendedRailType.isSavedRail && facingStart.isParallel(facingEnd)
                    ...
                    newRailType = ExtendedRailType.CABLE_CAR;
                }
            } else {
                isValidContinuousMovement = true;
                newRailType = extendedRailType;
            }
            ...
            switch (newRailType) {
                ...
                //case TURN_BACK:
                    ...
                    //break;
                default:
                    rail = Rail.newRail(positionStart, facingStart, positionEnd, facingEnd, newRailType.railShape, 0, new ObjectArrayList<>(), isOneWay ? 0 : newRailType.speedLimit, newRailType.speedLimit, false, false, newRailType.canAccelerate, newRailType == ExtendedRailType.RUNWAY, newRailType.hasSignal, transportMode);
            }
