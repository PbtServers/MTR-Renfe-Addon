package es.joseka.renfemtr.mod.render;


import es.joseka.renfemtr.mod.blocks.BlockStationNameCercaniasNewBase;
...
public class RenderStationNameTiledCercaniasNew <T extends BlockStationNameCercaniasNewBase.TileEntityStationNameCercaniasNewBase> extends BlockEntityRenderer<T> implements IGui, IDrawing {
    ...
    public RenderStationNameTiledCercaniasNew(Argument dispatcher, float maxWidth, float maxScale, float xOffset, float yOffset, float zOffset, float xTilt, int textColor, boolean isDoubleSided, String font) {
        super(dispatcher);
    ...
    public void render(@Nonnull T entity, float tickDelta, @Nonnull GraphicsHolder graphicsHolder, int light, int overlay) {
        final Style style = Config.getClient().getUseMTRFont() ? Style.getEmptyMapped().withFont(new Identifier(Init.MOD_ID, "vialog")) : Style.getEmptyMapped();
        ...
        final Direction facing = IBlock.getStatePropertySafe(state, BlockStationNameCercaniasNewBase.FACING);
